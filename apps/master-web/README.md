...
- `VITE_API_BASE_URL` — адрес backend (по умолчанию `http://localhost:3021`);
- `VITE_AROMA_WEB_URL` — адрес гостевого контура, который открывает кнопка
  «Витрина гостя» на экране рейлов (по умолчанию `http://localhost:5174`).
