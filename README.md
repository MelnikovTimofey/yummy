...
Репозиторий продукта «Арома Ателье» — каталог кальянных табаков и миксов.

## Поверхности

- `apps/aroma-web` — гостевой `Арома Ателье` (без авторизации);
- `apps/master-web` — backoffice `Мастер`;
- `apps/backend` — Node/TypeScript + Prisma + Postgres;
...
(`Tobacco`, `Mix`, `Rail`). Налить продуктовые данные можно двумя
...
Каталог залит — теперь поверх него собираются пул миксов и prepared-рейлы
...
Прогон идемпотентен; берёт в миксы только табаки «Выпускается» и в наличии,
раскладывает миксы по рейлам правилами из `docs/data/preset-rails.md` и
пропускает миксы с неактуальным или ненайденным компонентом (причины в логе).
...
поднять только БД через docker: `cd apps/backend && npm run db:start`).
...
- [`PRD.md`](PRD.md) — продуктовое видение «Арома Ателье».
- [`HANDOFF.md`](HANDOFF.md) — журнал последних значимых изменений.
- [`docs/atelier/`](docs/atelier/) — план реализации, roadmap, env-matrix, чек-листы.
