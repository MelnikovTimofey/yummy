...
> ТОЛЬКО в отдельной ветке `feature/<slug>` или `bug/<slug>` от `main`.
> Прямые правки в `main` (Edit/Write/коммит) запрещены — даже для «мелочей»,
> Dockerfile, конфигов, документации. Перед первым `Edit`/`Write` в сессии
> проверить ветку: `git rev-parse --abbrev-ref HEAD`. Если `main` — сначала
> `git checkout -b feature/<slug>`. Если сомневаешься в названии слага —
> спросить пользователя, а не уходить в `main`.
