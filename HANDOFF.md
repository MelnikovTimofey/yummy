...
> **Файл заморожен (25 мая 2026).** Это архив прошлых решений; новые
> записи сюда не добавляются. Действующие правила — в `CLAUDE.md`,
> «почему» свежих изменений — в commit message и PR description,
> incident-driven уроки для будущих сессий — в `MEMORY.md`. См. §8
> `CLAUDE.md`.

## 2.42) Process (25 мая 2026) — worktree-контракт для параллельных веток в §5

- Запрос: «нужно сформировать процесс, когда ты не мешаешь себе работать
  в разных ветках». До этого PR §5 формулировал только «писать в свою
  ветку от свежего main», но не описывал, как параллельные Claude-сессии
  не наступают друг другу на ноги, когда задач больше одной одновременно.

- Реализация (PR #47, ветка `feature/parallel-work-discipline`): второй
  жёсткий callout-блок в §5 CLAUDE.md сразу после существующего «не
  писать в main». Контракт **«одна сессия — одна ветка — одна рабочая
  копия»** из 5 пунктов:
  - **Один срез = один worktree** — параллельные задачи через
    `git worktree add ../nomad-<slug> -b feature/<slug>` (или `bug/` /
    `chore/`), а не через `git checkout` в одной рабочей копии.
    Переключение веток с грязным working tree утянет чужие правки в
    новую ветку и смешает scope.
  - **Anchor-чек на старте каждой сессии** — `pwd && git rev-parse
    --abbrev-ref HEAD && git status --short` перед первым
    `Edit`/`Write`. Если ветка не та или есть «чужие»
    untracked/modified файлы — стоп.
  - **Не переключать ветку внутри сессии** — `git checkout` другой
    ветки посреди сессии запрещён; задача меняется → закрыть текущую
    (commit или явный stash с пометкой) и начать новую сессию от
    свежего `main`.
  - **Edit/Write — только относительно cwd сессии** — в worktree-сессии
    абсолютный путь в основную копию запишет в `main`-копию, а не в
    worktree, и правка «потеряется» относительно ветки (это уже было
    зафиксировано в user-memory `feedback_worktree_abs_paths`).
  - **Закрывать worktree после merge** — `git worktree remove
    ../nomad-<slug>` сразу после merge PR. `git worktree list` —
    single source of truth о параллельных сессиях.

- Контекст триггера записи: прямо в этой же сессии после
  `git checkout -b feature/parallel-work-discipline` параллельная
  Claude-сессия успела переключить HEAD на `main` (видно по reflog:
  переходы на `feature/master-rails-hooks`, commit там, потом обратно
  на `main`). Я во втором turn'е начал сразу с `git add && commit`,
  пропустив anchor-чек, и положил коммит на `main` (4305e047). Откатил
  через `git branch -f feature/... HEAD` + `git update-ref
  refs/heads/main refs/remotes/origin/main` — коммит цел, main вернулся
  к origin/main. Это и есть тот класс ошибок, который правило закрывает:
  если бы anchor-чек был обязательным правилом на старте каждого turn'а,
  ошибки бы не случилось.

- Категория ревью: **process/workflow** — требует human review по
  `.github/NOMAD_REVIEW_POLICY.md`. Сам не мержу.

- Остаточный риск: правило процессное, продуктовая поверхность не
  затронута. CI gate в этом PR — только smoke и docs-check (все
  build-job'ы skipping). Двусмысленность пунктов 4 и 5 формулировок —
  ловится ревью.
