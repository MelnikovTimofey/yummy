...
Рекомендуемый следующий шаг для этого репозитория:

1. утвердить этот operating model как source of truth;
2. завести первый пакет repo-скиллов:
   - `yummy-repo-guard`
   - `nomad-backend-delivery`
   - `nomad-qa-and-smoke`
   - `repo-docs-and-handoff`
3. после этого подключать новые скиллы только под реально повторяющиеся workflows.
