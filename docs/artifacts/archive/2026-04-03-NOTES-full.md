Обновление от 3 апреля 2026 (nomad master: compact multi-select filters instead of tag walls):
- Проблема:
  - в `Inventory` и `Mixes` фильтры рендерились как длинные облака тегов, и при большом количестве производителей/вкусов экран начинался с шумного полотна вместо рабочей таблицы;
  - выбор фильтров был визуально тяжёлым: оператор видел сразу весь справочник, а не компактный control с возможностью быстро найти нужное значение.
- Изменение:
  - `apps/nomad-master-web/src/components/ui/filter-multi-select.tsx`:
    - добавлен общий searchable multi-select для фильтров;
    - control показывает компактный summary (`Все варианты` или выбранные значения), раскрывает dropdown-панель, поддерживает чекбоксы и `Очистить` для всей группы.
  ...
  - `apps/nomad-master-web/src/components/mixes/mix-catalog-view.tsx`
    - группы `Производители`, `Категории`, `Вкусы`, `Мета-теги` переведены со сплошных tag-cloud блоков на новый compact pattern;
    - `Inventory` и `Mixes` теперь ведут себя одинаково, без визуального расхождения между соседними рабочими поверхностями.
  - `apps/nomad-master-web/src/App.tsx`:
    - добавлены отдельные clear-handlers для сброса всей группы фильтров одним запросом, без последовательного toggle каждого значения.
  - `apps/nomad-master-web/src/styles.css`:
    - добавлены стили для trigger/panel/search/options состояния;
    - filter area стала компактнее и перестала съедать первый экран.
  - docs sync:
    - `HANDOFF.md`
    - `NOTES.md`
- Проверки:
...
- Эффект:
  - фильтры стали ближе к нормальному operational UI, а не к списку всех справочников сразу;
  - поиск по значениям теперь происходит внутри группы, что особенно важно для длинного списка вкусов и производителей;
  - change не меняет продуктовые правила фильтрации, только способ выбора и сброса значений.

Обновление от 3 апреля 2026 (nomad master: separate mix creation screen):
- Проблема:
  - создание нового микса продолжало жить внутри того же `manager-layout`, что и каталог, из-за чего форма занимала место таблицы, визуально смешивалась с edit-flow и на некоторых ширинах ломала сетку компонентных инпутов;
  - операторский сценарий `просмотреть каталог -> открыть создание` оставался неявным: пустой editor-panel выглядел как недоделанная форма, а не как отдельный экран действия.
- Изменение:
  - `apps/nomad-master-web/src/App.tsx`:
    - добавлен отдельный screen-state для `mixes`: `catalog` и `create`;
    - создание нового микса теперь открывается как самостоятельный экран внутри вкладки `Миксы`, а после успешного сохранения UI возвращается в каталог;
    - редактирование существующего микса по-прежнему остаётся рядом с таблицей как быстрый operational flow.
  - `apps/nomad-master-web/src/components/mixes/mix-catalog-view.tsx`:
    - каталог и create-flow разведены на разные представления;
    - в каталоге вместо пустой create-формы показывается короткий placeholder с CTA `Создать микс`;
    - full-width экран создания получил собственный header, отдельное действие `Вернуться в каталог` и ту же форму без привязки к table shell.
  - `apps/nomad-master-web/src/styles.css`:
    - добавлены styles для `mixes-create-screen` и placeholder editor-state;
    - строки компонентов переведены на более устойчивую сетку с actions на отдельной строке, чтобы селект, доля и управляющие кнопки не наезжали друг на друга.
  - docs sync:
...
- Проверки:
  ...
  - browser pass через `[$playwright](/Users/admin/.codex/skills/playwright/SKILL.md)` на `http://localhost:5177`:
    - login `admin/admin`
    - переход в `Миксы`
    - открытие отдельного экрана `Создание микса`
    - добавление компонента и визуальная проверка layout формы
- Эффект:
  - создание микса перестало конкурировать за место с каталогом и считывается как отдельное действие, а не как случайно раскрытая правая колонка;
  - форма создания стала устойчивее по сетке и больше не разваливает поля компонентов в проверенном браузерном runtime;
  - change остаётся frontend-only и не меняет contract/backend semantics.

Обновление от 2 апреля 2026 (nomad master: UX audit layout tightening and navigation reset):
- Проблема:
  - после первого redesign pass shell всё ещё терял рабочую ширину из-за тяжёлого sidebar, поэтому `Dashboard` и CRUD-модули начинали экран с лишнего chrome вместо полезного контента;
  - дашборд и другие экраны содержали слишком много поясняющего copy, из-за чего заголовки и operational cues тонули в описательном тексте;
  - filters/toolbars в `Inventory` и `Mixes` расползались и конфликтовали по ширине, а часть stats/form blocks растягивалась по высоте сильнее, чем нужно;
  - palette ушла слишком далеко в нейтральный beige/UI и потеряла семейное сходство с `Арома Ателье`;
  - на рабочих вкладках оставалось слишком много пустого места: global KPI strip дублировал локальные module stats, а filter groups получали лишнюю высоту;
  - mix editor оставался внизу каталога, что ломало рабочий сценарий "нашёл строку -> сразу отредактировал".
- Изменение:
  - `apps/nomad-master-web/src/App.tsx`:
    - широкий sidebar заменён на верхний shell с brand/identity/status row и horizontal navigation menu;
    - stage header и module summary strip уплотнены, а длинные navigation descriptions убраны;
    - global KPI strip теперь остаётся только на `Dashboard`, чтобы рабочие вкладки начинались сразу с module surface.
    - login screen сохранён как отдельный split-layout без возврата к старому stacked intro.
  - `apps/nomad-master-web/src/components/dashboard/dashboard-view.tsx`:
    - заголовок и intro copy сокращены до utility-level текста;
    - убраны лишние descriptive paragraphs и англоязычные labels в верхнем аналитическом слое;
    - hero и dashboard cards дополнительно уплотнены по вертикальному ритму.
  - `apps/nomad-master-web/src/components/inventory/inventory-view.tsx`
  - `apps/nomad-master-web/src/components/mixes/mix-catalog-view.tsx`
    - сокращены helper/descriptive тексты в inventory/mixes surfaces;
    - editor миксов перенесён из нижней части страницы в отдельную рабочую колонку рядом с каталогом.
  - `apps/nomad-master-web/src/styles.css`:
    - пересобран shell-level layout под top navigation вместо sidebar;
    - palette смещена к muted burgundy/amber baseline по мотивам `Арома Ателье`, но без превращения `Master` в guest-style UI;
    - filters/toolbars переведены из loose auto-fit в более жёсткую desktop-сетку, чтобы поля и select'ы не наезжали друг на друга;
    - filter groups переведены в wrap-layout без равновысотных пустых контейнеров;
    - уменьшены padding/min-height у stats, form surfaces и secondary blocks;
    - `mixes` workspace переведён в master-detail layout со sticky editor column.
  - docs sync:
    - `apps/nomad-master-web/README.md`
    - `HANDOFF.md`
    - `NOTES.md`
- Проверки:
  - `cd apps/nomad-master-web && npm test`
  - `cd apps/nomad-master-web && npm run build`
  - browser pass через `[$playwright](/Users/admin/.codex/skills/playwright/SKILL.md)` против `http://localhost:5177`
  - `git diff --check`
- Эффект:
  - `Master` перестал терять полезную ширину на shell-level chrome и быстрее считывается как рабочее меню, а не как витрина;
  - filters и stats стали компактнее и предсказуемее на desktop, а toolbar больше не разваливается на налезающие или случайно перенесённые controls;
  - визуальный тон снова читается как Nomad-family surface: ближе к `Арома Ателье`, но тише и функциональнее;
  - сценарий редактирования миксов стал ближе к реальной работе оператора: каталог и форма теперь живут рядом;
  - change остаётся frontend-only UX slice без backend или contract drift.
...
Обновление от 3 апреля 2026 (Nomad — подготовка HTReviews integration foundation):
- `apps/nomad-backend/src/integrations/htreviews/*`:
  - добавлен изолированный HTML adapter для `https://htreviews.org`;
  ...
  - snapshot содержит:
    ...
    - country/status/strength;
    - raw tags;
    - candidate split в Nomad taxonomy (`flavorProfiles`, `flavors`, `flavorTags`).
- `apps/nomad-backend/scripts/import-htreviews.ts`, `package.json`:
  - добавлен CLI dry-run import `npm run import:htreviews:preview`;
  - preview сохраняется в JSON и не меняет runtime-каталог автоматически.
- `apps/nomad-backend/src/integrations/htreviews/parser.test.ts`:
  - добавлены unit tests на brand index, brand page, tobacco detail и candidate taxonomy.
- `apps/nomad-backend/README.md`:
  - добавлен runbook по preview import и ограничениям текущего foundation-среза.

Проверка:
- см. текущий handoff / final report по фактически прогнанным командам.

Обновление от 3 апреля 2026 (Nomad — HTReviews sync в текущую БД):
- `apps/nomad-backend/prisma/schema.prisma`:
  - `NomadTobacco` расширен под импортируемый каталог:
    - `lineName`
    - `alias`
    - `sourceKind`
    - `sourceUrl`
    ...
    - `imageUrl`
    - `rawSourceTags`
  - уникальность изменена с `manufacturer + name` на `manufacturer + lineName + name`, чтобы HTReviews line-based catalog не конфликтовал внутри бренда.
- `apps/nomad-backend/src/integrations/htreviews/sync.ts`, `scripts/sync-htreviews.ts`, `package.json`:
  ...
  - sync делает upsert в `NomadTobacco` и использует безопасный default `inStock=false` для новых импортов;
  - повторный прогон идемпотентен по source IDs и fallback `brand + line + name`.
- `apps/nomad-backend/src/state.ts`:
  - inventory view теперь несёт `lineName`, и lineName участвует в backend search haystack.
- `apps/nomad-backend/README.md`:
  - добавлен runbook для sync в текущую Nomad DB.
- live-run в текущую БД:
  - discovery расширен с одного `/tobaccos/brands` до union публичных entrypoint'ов:
    - `/tobaccos`
    - `/tobaccos/new`
    - `/tobaccos/brands`
    - `/tobaccos/lines`
  - current DB после live sync содержит:
    - `1672` tobacco rows total;
    - `1658` rows с `sourceKind='htreviews'`;
    - все импортированные строки имеют заполненный `lineName`;
    - новые импортированные позиции выставлены как `inStock=false`.

Обновление от 3 апреля 2026 (Nomad — очистка seed/test inventory и live rebuild каталога):
- `apps/nomad-backend/src/db.ts`, `package.json`, `src/state.ts`:
  - backend tests уведены в отдельную Prisma schema `nomad_test`;
  - `npm test` теперь делает `prisma db push` в `nomad_test` и не должен перетирать рабочую `public`;
  - `resetNomadState()` запрещён вне test-mode без явного `NOMAD_ALLOW_STATE_RESET=1`.
- `apps/nomad-backend/src/integrations/htreviews/catalog.ts`:
  - import path стал skip-safe по отдельным битым HTReviews URL, чтобы один проблемный page не валил весь sync.
- `apps/nomad-backend/scripts/rebuild-live-catalog.ts`, `package.json`, `README.md`:
  - добавлен операционный script `npm run rebuild:live-catalog`;
  - script:
    - очищает `public` schema от seed/test product-data;
    - удаляет seed tobacco;
    - обновляет HTReviews tobacco;
    - пересобирает `11` mixes и `3` rails поверх реальных HTReviews tobacco;
    - выставляет `inStock=true` только для выбранного рабочего ассортимента mix components.
- фактический live-state после rebuild:
  - `1674` tobacco rows total;
  - `1674` rows с `sourceKind='htreviews'`;
  - `0` seed tobacco rows;
  - `14` rows `inStock=true`;
  - `11` mixes и `3` rails живут на HTReviews tobacco IDs.
- фактические component mappings после rebuild включают, например:
  - `mix-citrus-scout` -> `Hit / Citrus Rock` + `MUSTHAVE / Peppermint`
  - `mix-apple-wave` -> `Mattpear / Ice GrannySmit` + `MUSTHAVE / Peppermint`
  - `mix-rose-afterglow` -> `Element / Chinese rose` + `Helix / Coconut Icecream`

Обновление от 3 апреля 2026 (Nomad — атрибут крепости из HTReviews в inventory):
- `apps/nomad-backend/src/state.ts`:
  - `InventoryTobaccoView` теперь отдаёт `officialStrength` и `communityStrength`;
  - search haystack staff inventory включает оба поля крепости.
- `apps/nomad-master-web/src/contracts.ts`, `src/components/inventory/inventory-view.tsx`:
  - master inventory нормализует и показывает крепость рядом с табаком;
  - при наличии отображаются обе оценки: официальная и пользовательская с HTReviews.

Обновление от 3 апреля 2026 (Nomad — HTReviews detail backfill вкусов и крепости):
- `apps/nomad-backend/src/integrations/htreviews/taxonomy.ts`:
  - taxonomy builder больше не выкидывает реальные вкусы при derivation profile;
  - `flavors` теперь сохраняют конкретные HTReviews taste tags (`лимон`, `мята`, `сливочный`, `чай`, `виски` и т.д.), а `flavorProfiles`/`flavorTags` остаются производными слоями.
- `apps/nomad-backend/scripts/backfill-htreviews-details.ts`, `package.json`, `README.md`:
  - добавлен операционный script `npm run backfill:htreviews:details`;
  - script проходит по существующим `sourceKind='htreviews'` rows, тянет detail page и обновляет:
    - `officialStrength`
    - `communityStrength`
    - `rawSourceTags`
    - `flavorProfiles`
    - `flavors`
    - `flavorTags`
  - после backfill script пересчитывает taxonomy существующих `NomadMix` по компонентам.
- фактический live-state после backfill в `public`:
...
- пример:
  - `База / Полегче / Сливочное масло` теперь имеет:
    - `officialStrength: Средняя`
    - `communityStrength: Средне-лёгкая`
    - `flavorProfiles: ["dessert","sweet"]`
    - `flavors: ["сливочный"]`

Обновление от 3 апреля 2026 (Nomad — карточка табака в master inventory):
- `apps/nomad-backend/src/state.ts`, `src/inventory.test.ts`:
  - inventory view теперь отдаёт дополнительные tobacco-атрибуты для staff UI:
    - `country`
    - `productionStatus`
    - `description`
- `apps/nomad-master-web/src/contracts.ts`, `src/contracts.test.ts`, `src/components/inventory/inventory-view.tsx`:
  - по клику на табак в inventory открывается popup-карточка;
  - карточка показывает описание, атрибуты, категории, вкусы, мета-теги и полный список зависимых миксов;
  - из карточки доступны быстрые действия:
    - смена наличия;
    - добавление/удаление из batch selection;
    - переход к зависимому миксу.

Обновление от 3 апреля 2026 (Nomad — HTReviews fallback для пустых tobacco-атрибутов):
- `apps/nomad-backend/src/integrations/htreviews/description.ts`, `src/integrations/htreviews/description.test.ts`:
  - добавлен factual fallback для `description`, если HTReviews page не содержит отдельного текстового блока;
  - fallback собирается из HTReviews tags, strength и status без ручного редактирования rows.
- `apps/nomad-backend/src/integrations/htreviews/parser.ts`, `src/integrations/htreviews/parser.test.ts`:
  - `communityStrength` теперь может быть выведена из шкалы `N/5` внутри HTReviews description, если поле `Крепость по оценкам` отсутствует на странице.
- `apps/nomad-backend/scripts/backfill-htreviews-details.ts`, `README.md`:
  - backfill получил точечные режимы:
    - `HTREVIEWS_ONLY_MISSING_DESCRIPTION=1`
    - `HTREVIEWS_ONLY_INCOMPLETE=1`
- фактический live-state в `public` после точечного backfill:
  - `0` пустых `description`;
  - `0` пустых `country`;
  - `0` пустых `officialStrength`;
  - `0` пустых `communityStrength`;
  - `0` пустых `productionStatus`.

Обновление от 3 апреля 2026 (Nomad — pagination для staff inventory и mixes):
- `apps/nomad-backend/src/state.ts`, `src/app.ts`, `src/inventory.test.ts`, `src/content.test.ts`:
  - `GET /staff/inventory/tobaccos` и `GET /staff/mixes` теперь поддерживают opt-in pagination через `page` и `pageSize`;
  - pagination включается только когда параметры переданы явно, поэтому служебные справочники для editor surfaces не обрезаются до первой страницы.
- `apps/nomad-master-web/src/contracts.ts`, `src/contracts.test.ts`, `src/App.tsx`:
  - frontend contract расширен pagination meta (`page`, `pageSize`, `totalPages`, `hasNextPage`, `hasPreviousPage`);
  - inventory/mixes list requests теперь ходят постранично и сбрасывают страницу на `1` при изменении поиска, фильтров и сортировки.
- `apps/nomad-master-web/src/components/inventory/inventory-view.tsx`, `src/components/mixes/mix-catalog-view.tsx`, `src/components/ui/list-pagination.tsx`, `src/styles.css`:
  - под staff-таблицами добавлен pager;
  - текстовый search в `Inventory` и `Mixes` переведён на debounce, чтобы не отправлять запрос на каждый символ.
