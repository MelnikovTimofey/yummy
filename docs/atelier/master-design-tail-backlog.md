...
```bash
git worktree add /Users/admin/PycharmProjects/atelier-<slug> -b <type>/<slug> origin/main
```
...
- `apps/master-web/src/components/shell/master-sort-pill.tsx` —
  добавить prop `direction: SortDirection` + `onDirectionChange`.
  Рендерить `ArrowUp` / `ArrowDown` lucide-icon рядом с chevron.
- `apps/master-web/src/components/shell/master-sort-pill.helpers.ts` —
...
- `apps/backend/prisma/schema.prisma` — у модели `DailyAccessCode`
  (или как она называется) есть `chatsTotal` / `chatsConfirmed`? Грепни.
  Если нет — добавить.
- `apps/backend/src/app.ts` или route-файл (`access.ts`?) —
  endpoint возвращает
  `windowHistory: [{ codeValue, expiresAt, chatsConfirmed, chatsTotal }]`
  (последние 5).
- `apps/backend/src/access.test.ts` — TDD на endpoint.
- Mock / seed: в `prisma/seed.ts` заполнить history для нескольких окон.
- Frontend:
  `apps/master-web/src/components/access/telegram-bot-status-block.tsx`
